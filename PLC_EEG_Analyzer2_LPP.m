...
%         
        %Epoching
        epochname = strcat('PLC_Sub',num2str(s),'block',num2str(b),'epochsLPP');
        EEG = pop_epoch( EEG, {1 2 501 502}, [-0.1   0.75], 'newname', epochname, 'epochinfo', 'yes'); %the onset of first gabor(target) is time 0
...
for s = allsubs
    ...
    for b = bs
        ...
        EEG = pop_eegthresh(EEG,1,97:99,-75,75,-0.1,0.75,0,0); %On EX3, EX4 and EX5, mark trials outside -75 to 75 uV
        %     end
        ...
        eval(['save PLC_EEG_Sub' num2str(s) '_block' num2str(b) '_eyerejtriallocs_lpp eyerejtriallocs']); %Save rejected trial locations
...
for s = allsubs
    ...
    for b = bs
        ...
        EEG = pop_eegthresh(EEG,1,255:257,-75,75,-0.1,0.75,0,0); %On EX3, EX4 and EX5, mark trials outside -75 to 75 uV
        %     end
...
%% subjects
% +- 90 uV for marking rejected trials for Sub16,37,54
...
%saved 2/7/15
save PLC_EEG_eegintrejtrials_LPP_norm.mat allsubs_feareegintrejtrials %saving info for this "normal" group of pariticipants with correct setup
save PLC_EEG_ChanInfo_LPP_norm.mat fearallchaninfo
...
%saved 2/7/15
save PLC_EEG_eegintrejtrials_Sub124647.mat allsubs_feareegintrejtrials %saving info for this "normal" group of pariticipants with correct setup
save PLC_EEG_ChanInfo_Sub124647.mat fearallchaninfo
...
for s = allsubs
    ...
    for b = bs
        ...
        for v = thesections
            ...
            eval(['load PLC_EEG_Sub' num2str(s) '_block' num2str(b) '_eyerejtriallocs_lpp eyerejtriallocs']); %Load up eyechan marked trials info (in 1 row)
            ...
            switch b
                ...
                case(5)
                    eval(['allsubs_fearall' num2str(v) 'rejtrials{frow' num2str(v) ',10} = length(allfrejtrials' num2str(v) ');']); %Col 6 = # of rejected trials for block 3
                    eval(['allsubs_fearall' num2str(v) 'rejtrials{frow' num2str(v) ',11} = allfrejtrials' num2str(v) ';']); %Col 11 = locations of rejected trials for block 3
                case(6)
                    eval(['allsubs_fearall' num2str(v) 'rejtrials{frow' num2str(v) ',12} = length(allfrejtrials' num2str(v) ');']); %Col 8 = # of rejected trials for block 4
                    eval(['allsubs_fearall' num2str(v) 'rejtrials{frow' num2str(v) ',13} = allfrejtrials' num2str(v) ';']); %Col 12 = locations of rejected trials for block 4
...
save PLC_EEG_LPP_finalallrejtrials_noica allsubs_fearfinalallrejtrials
save PLC_EEG_LPP_rejtypetally_noica allrejtypetally
...
for s = allsubs
    ...
    for b = bs
        ...
        for v = thesections
            ...
            eval(['load PLC_EEG_Sub' num2str(s) '_block' num2str(b) '_allrejlogical' num2str(v) '_lpp rejectlogical']);
...
%                 
%             else
            filenamef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'epochsLPP_reref_blc.set'); %Dataset free of previous thresholding
 %           end
            ...
            if v=='B'
                savefilef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'_epochsLPP_blc_Barej.set');
            elseif v=='M'
                savefilef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'_epochsLPP_blc_Marej.set');
            elseif v=='F'
                savefilef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'_epochsLPP_blc_Farej.set');
            else
