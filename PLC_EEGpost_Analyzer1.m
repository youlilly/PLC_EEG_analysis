%PLC_EEGpost_Analyzer1.m
%Created by YY, 7/18/14
...
% Note: all T2 post test EEG files are saved int he folder PLC_EEG_T2
% In the first step, converted files are named without the "post" suffix

allsubs = 57 %[40 42:44 46:52 54:55];

%Starting from sub40-57
Subinitials = ['QL'; 
    'DS';
    'AD';
    'LN';
...
for s = allsubs
    initial = Subinitials(s-39,:);
    bs = 4:6;
    
    for b = bs %For each block included in these analyses
        
    filename = strcat('Sub', num2str(s),'_', initial, '_block',num2str(b),'post-Deci.bdf');
...
for s = allsubs %For each of the 4 blocks
    ...
    filename1 =strcat('PLC_EEG_block',num2str(b),'_post_sub',num2str(s),'_',initial,'.mat');
    load(filename1);
...
save PLC_beheegdiffs_post numtrigdiffs

%% Label events in EEG file and adjust onset times, both based on info from .mat file.

%allsubs = [1:4 7 9 10 12 13 15:18 20:25 27 28 32:35 37:40 42:44 46:52 54 55 57];%Subjects whose EEG has been collected as retest
allsubs = [ 9 10 12 13 15:18 20:25 27 28 32:35 37:40 42:44 46:52 54 55 57];%Subjects whose EEG has been collected as retest
...
%%%Selectively remove initial triggers based on individual subjects
    ...
    elseif s == 7 && b == 5 
        cutoff = 768;
    elseif (s == 4 && b == 4) || (s == 18 && b == 5) || (s == 32 && b == 5)
        cutoff = 3072;
    elseif (s == 21 && b == 5) || (s == 7 && b == 4)
        cutoff = 2560;
    elseif s == 24 && b == 5
        cutoff = 512;
    elseif s == 33 && b == 6
...
counter2 = 0;
    ...
    savefileb = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt.set'); %Add "_evt" to filename
...
for s = allsubs
    ...
    for b = bs
    %for n = allsubs
    filename4=strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt.set');
    ...
    EEG = eeg_checkset( EEG );
    savefile4 = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil.set'); %Add "_fil" to filename
    EEG = pop_saveset(EEG, savefile4);
    clear EEG %clear dataset in memory to avoid overloading it
    end
end

% %% Relabel events based on correct/incorrect answers
% % Last ran 7/25/12
% allsubs = [1:3 5:6 8:15 17:22 24 25]; 
% 
% for e= 1:4
%     
%     for m = [31 36] %allsubs
% 
%     eval(['load LHE_block' num2str(e) '_sub' num2str(m) '_results']);
%  
%     %Load up EEG file
%     filename4a=strcat('LHEsub',num2str(m),'block',num2str(e),'_evt_fil.set');
%     EEG = pop_loadset(filename4a); %Load up EEG dataset
%     
%     if e==3 && m==6
%         j = 298;
%     else
...
%     end
%     
%         for l=1:j % for each trial in this block
%             for la=1:length(incorrect_trials)
%                 if l == (incorrect_trials(la)*2-1) || l==incorrect_trials(la)*2
%                  
%                  EEG.event(l).type = EEG.event(l).type + 6; %If this trial is incorrect, the event type will increase 6
%                  
%                 end
%                 
%             end
%             
%             for lb=1:length(noresp_trials)
%                 if l == (noresp_trials(lb)*2-1) || l==noresp_trials(lb)*2
%                  
%                  EEG.event(l).type = EEG.event(l).type + 12; %If this trial is noresp, the event type will increase 12
%                                      
%                 end
%                 
%             end
...
%             if rem(l,2)==0
%                 
%                EEG.event(l).type = 99;%If this is the bothon trigger, label the event to be type99; updated Dec 31,2011      
%                
%             end
%             
%         end
%       
%         
%     savefile4a = strcat('LHEsub',num2str(m),'block',num2str(e),'_evt_fillab.set'); %Add "_evt" to filename
%     EEG = pop_saveset(EEG, savefile4a); %Resave
%     EEG = eeg_checkset( EEG ); %#ok<NASGU>
% %     
%     clear EEG %To keep Matlab from screaming
%  
