...
%1st visit
%allsubs = [1:2 7 9 10 12 13 15:18 20][21:25 27 28 32:35 37:40 44 46:52 54 55 57];%Subjects whose EEG has been collected as retest
...
%               
        ...
        savefile2 = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc.set');
...
%% Remove channel labels and data from 97 onwards; Run ICA 

icasubs = [2 7 16 33 37 38 54 55];
...
for s = icasubs
    ...
    for b = bs
        ...
        filename = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil.set');
        ...
        savefile = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil_ica_chanrm.set');
        EEG = pop_saveset( EEG,  'filename', savefile); %Save file in .set format
    end
end

%% Reload channel loc file "elp96test"; 
icasubs = [2 7 16 33 37 38 54 55];
...
for s = icasubs
    ...
    for b = bs
        ...
        filename = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil_ica_chanrm.set');
        ...
        %    EEG = pop_runica(EEG, 'icatype', 'runica', 'chanind', 1:96);
        savefile = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil_ica_chanld.set');%channel loc file loaded
        EEG = pop_saveset( EEG,  'filename', savefile); %Save file in .set format
    end
end
...
bs = 4:6;
    ...
    for b = bs
        filename = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil_ica_chanld.set');
        
        
        EEG = pop_loadset(filename);
        EEG = pop_topoplot(EEG, 0, 1:30, 'ICA component maps');
        eval(['saveas(gcf,''Sub' num2str(s) 'block' num2str(b) 'post_ICAcomponents.tif'');']); %Example saved file: "grandavgchanA15best.ai"
...
for s = icasubs
    ...
    for b = bs
        ...
        for c = 1:length(cp)
            ...
            eval(['saveas(gcf,''Sub' num2str(s) 'block' num2str(b) 'post_ICAcomponents' num2str(cn) '.tif'');']); %Example saved file: "grandavgchanA15best.ai"
            close(gcf); %Close the figure you just graphed, so you don't end up with a million figure windows all open at once
        end
    end
...
for s = icasubs
    ...
    for b = bs
        ...
        savefile = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_evt_fil_ica_rm.set');
        EEG = pop_saveset( EEG,  'filename', savefile); %Save file in .set format        
    end
...
%% Threshold each channel and save marked-trials info- For ICA-treated subjects
...
for s = allsubs
    ...
    for b = bs
        filename6 = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc.set');
        EEG = pop_loadset(filename6); %Load up baseline-corrected data
        allrejtrialtest = cell(101,3); %Col 1 = chan #, col 2 = # of rejected trials for that chan, col 3 = trial #s flagged for rejection for that chan
        ...
        eval(['save PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_allrejtrialtest_ica allrejtrialtest']);
        clear EEG
...
%% Exclude channels, save corresponding info, and mark artifacts on just
%% the included EEG channels. - ICAed subjects
allsubs = [2 33 38 55];
...
for s = allsubs
    ...
    for b = bs
        ...
        eval(['load PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_allrejtrialtest_ica.mat']); %Load up all marked trials info
        ...
        chaninfo.chanexc=fchanexc;
        chaninfo.chaninc=fchaninc;
        eval(['save PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_chaninfo_ica chaninfo']); %Save excluded & included channels matrices
        ...
        %Mark artifacts on just the included channels of interest.  In 3
        %batches, one for each section of the head.
        filename4 = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc.set'); %Dataset free of previous thresholding
        EEG = pop_loadset(filename4);
        ...
        Bintrejtriallocs = find(EEG.reject.rejglobal); %Obtain rejected trial #s
        eval(['save PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_Bintrejtriallocs_ica Bintrejtriallocs']); %Save rejected trial locations
        ...
        Mintrejtriallocs = find(EEG.reject.rejglobal); %Obtain rejected trial #s
        eval(['save PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_Mintrejtriallocs_ica Mintrejtriallocs']); %Save rejected trial locations
        ...
        Fintrejtriallocs = find(EEG.reject.rejglobal); %Obtain rejected trial #s
        eval(['save PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_Fintrejtriallocs_ica Fintrejtriallocs']); %Save rejected trial locations
...
%saved 2/8/15
% save PLC_EEGpost_eegintrejtrials_ICAsubs.mat allsubs_feareegintrejtrials
% save PLC_EEGpost_ChanInfo_ICAsubs.mat fearallchaninfo
...
for s = allsubs
    ...
    for b = bs
        ...
        for v = thesections
            ...
            eval(['load PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_' num2str(v) 'intrejtriallocs_ica ' num2str(v) 'intrejtriallocs']); %Load up EEGchan-of-interest marked trials info (in 1 row)
            ...
            rejectlogical = logical(rejectbinary); %#ok<NASGU> %Converts the zeros-and-ones matrix to a logical for use in EEG.reject.rejglobal
            eval(['save PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_allrejlogical' num2str(v) '_ica rejectlogical']);
        end
    end
    frowB = frowB + 1; %Move to next row
    frowM = frowM + 1;
...
for s = allsubs
    ...
    for b = bs
        ...
        for v = thesections
            ...
            eval(['load PLC_EEGpost_Sub' num2str(s) '_block' num2str(b) '_allrejlogical' num2str(v) '_ica rejectlogical']);
            if s==3||s==4||s==42||s==43
                ...
                filenamef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc.set'); %Dataset free of previous thresholding
            end
            ...
            if v=='B'
                savefilef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc_Barej.set');
            elseif v=='M'
                savefilef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc_Marej.set');
            elseif v=='F'
                savefilef = strcat('EEG_PLC_Sub',num2str(s),'block',num2str(b),'post_ica_epochs_blc_Farej.set');
            else
